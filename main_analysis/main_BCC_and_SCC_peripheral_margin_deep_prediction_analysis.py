...
def main(argv):
    ### DATA PRE PROCESSING
    dataset = getDataFromCsv('./datasets/nhs-data/[DATASET_3]_dataset_nhs_peripheral_margin_deep_prediction.csv')
    input = dataset[1]
    classData = dataset[2]

    data = PreProcess(input, classData, datasetName="bcc_scc_margin_deep")
    ...
    if len(argv) == 3 and argv[2] == "cv":
        ...
        autoML.cross_validation_process("./outputCrossValidationAutoML/BCC_SCC_margin_deep/")
        autoML.show_latex_cross_validation_process()
    elif len(argv) == 4 and argv[3] == "weka":
        autoML.k_folds_split(k_folds=10)
        autoML.cross_validation_process("./outputCrossValidationAutoML/BCC_SCC_margin_deep/", AutoSklearn=False)
    else:
        autoML.fit_predict()
